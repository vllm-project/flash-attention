...
def _flash_attn_fwd(
    ...
    if fp8_kv_dequant:
        ...
        assert qv is None, "fp8_kv_dequant is not supported with Qv/MLA"
        assert cp_world_size == 1 and cp_tot_seqused_k is None, (
            "fp8_kv_dequant is not supported with context parallelism"
        )
        assert num_splits_dynamic_ptr is None, (
            "fp8_kv_dequant is not supported with dynamic split scheduling"
        )
    else:
