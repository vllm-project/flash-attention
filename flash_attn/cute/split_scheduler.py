...
def plan_hopper_split_schedule(
    ...
    *,
    ...
    """Return FA3-style per-request SplitKV counts for measured Hopper decode."""
    ...
    if (
        q_lens.numel() == 0
        or q_lens.numel() > 128
        or (has_qv and not all_single_token)
        or (not has_qv and not (all_single_token or mixed_decode_extend))
        or cp_world_size != 1
        or fast_build
    ):
        return None

    max_seqlen_k = seq_lens_cpu.max().item()
    min_seqlen_k = seq_lens_cpu.min().item()
    mla_dv512_decode = (
        has_qv
        and num_heads_kv == 1
        and head_dim == 64
    ...
    if (
        mla_dv512_h64_batch16
        and min_seqlen_k == max_seqlen_k == 512
        and cuda_graph_max_num_splits is None
    ):
        return SplitSchedulePlan(num_splits=4, split_counts=None)
    graph_stable_mla = (
        cuda_graph_max_num_splits is not None and mla_dv512_decode
    )
    if (
        mla_dv512_h64_batch16
        and cuda_graph_max_num_splits is not None
        and cuda_graph_max_num_splits < 8
    ):
        return None
    if has_qv:
        if cuda_graph_max_num_splits is not None:
            eligible = head_dim_v == 256 or graph_stable_mla
        else:
            eligible = head_dim_v == 256 or (
                head_dim_v == 512
                and (
                    min_seqlen_k != max_seqlen_k
                    or max_seqlen_k >= 8192
                    or (q_lens.numel() == 1 and max_seqlen_k >= 4096)
                    or (
                        q_lens.numel() == 64
                        and max_seqlen_k <= 512
                        and num_heads_q in (16, 32)
                    )
                )
            )
        tile_m, tile_n = 64, 64
    else:
        eligible = head_dim == head_dim_v == 256
    ...
    num_n_blocks = [
        math.ceil(kv_len / tile_n) for kv_len in seq_lens_cpu.tolist()
    ]
    total_blocks = sum(
        m_blocks * n_blocks
    ...
    if keep_all_one_schedule:
        if graph_stable_mla:
            ...
            min_active_m_blocks = min(
                blocks for blocks in num_m_blocks if blocks > 0
            )
            graph_split_bound = min(
                max_num_splits,
                math.ceil(
                    num_sms
                    / (1.1 * num_heads_kv * min_active_m_blocks)
                ),
            )
