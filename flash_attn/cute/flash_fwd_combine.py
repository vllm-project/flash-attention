...
class FlashAttentionForwardCombine:
    ...
    def kernel(
        ...
        if const_expr(num_splits_dynamic_ptr is not None):
            num_splits = num_splits_dynamic_ptr[maybe_virtual_batch]
            if const_expr(self.skip_single_split):
                num_splits = cutlass.max(
                    1,
                    cutlass.min(num_splits, mLSE_partial.shape[1]),
                )
        else:
            num_splits = mLSE_partial.shape[1]
