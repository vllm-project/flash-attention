...
template<typename T, bool Is_causal>
void run_mha_bwd_hdim40(Flash_bwd_params &params, cudaStream_t stream) {
    constexpr static int Headdim = 40;
    int device;
    cudaGetDevice(&device);
    int max_smem_per_block;
    cudaError status_ = cudaDeviceGetAttribute(
...
template<typename T, bool Is_causal>
void run_mha_bwd_hdim72(Flash_bwd_params &params, cudaStream_t stream) {
    constexpr static int Headdim = 72;
    int device;
    cudaGetDevice(&device);
    int max_smem_per_block;
    cudaError status_ = cudaDeviceGetAttribute(
...
template<typename T, bool Is_causal>
void run_mha_bwd_hdim80(Flash_bwd_params &params, cudaStream_t stream) {
