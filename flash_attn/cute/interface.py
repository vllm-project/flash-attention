...
def _flash_attn_fwd(
    ...
    if use_dynamic_splits:
        assert arch // 10 == 9, "dynamic split counts are only supported on SM90"
    ...
    if compile_key not in _flash_attn_fwd.compile_cache:
        ...
        final_o_tensor = (
            to_cute_tensor(out) if use_dynamic_split_varlen else None
        )
        final_lse_tensor = (
            to_cute_tensor(lse, assumed_align=4)
            if use_dynamic_split_varlen
            else None
        )
    ...
    if not is_fake_mode():
        ...
        if qv is not None and arch // 10 in [10, 11]:
            ...
            if arch // 10 == 9:
                ...
                call_args.append(out_call if use_dynamic_split_varlen else None)
                call_args.append(lse if use_dynamic_split_varlen else None)
    ...
    if is_split_kv:
        _flash_attn_fwd_combine(
            ...
            skip_single_split=use_dynamic_split_varlen,
