...
STABLE_TORCH_LIBRARY(_vllm_fa3_C, m) {
    m.def("fwd("
        ...
        "int cp_world_size,"
        "int cp_rank,"
        "Tensor? cp_tot_seqused_k) -> (Tensor(out!), Tensor, Tensor, Tensor)");
    m.def("bwd("
        "Tensor dout,"
        "Tensor q,"
        "Tensor k,"
        ...
        "float softcap,"
        "bool deterministic,"
        "int sm_margin) -> (Tensor(dq!), Tensor(dk!), Tensor(dv!), Tensor, Tensor, Tensor, Tensor, Tensor)");
    m.def("fwd_combine("
        "Tensor out_partial,"
        "Tensor lse_partial,"
        "Tensor(out!)? out,"
        "ScalarType? out_dtype) -> (Tensor(out!), Tensor)");
