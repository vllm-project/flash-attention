...
DISABLE_HDIM40 = os.getenv("FLASH_ATTENTION_DISABLE_HDIM40", "FALSE") == "TRUE"
DISABLE_HDIM64 = os.getenv("FLASH_ATTENTION_DISABLE_HDIM64", "FALSE") == "TRUE"
DISABLE_HDIM72 = os.getenv("FLASH_ATTENTION_DISABLE_HDIM72", "FALSE") == "TRUE"
DISABLE_HDIM80 = os.getenv("FLASH_ATTENTION_DISABLE_HDIM80", "FALSE") == "TRUE"
...
COMPILED_HDIMS = (
    []
    + ([40] if not DISABLE_HDIM40 else [])
    + ([64] if not DISABLE_HDIM64 else [])
    + ([72] if not DISABLE_HDIM72 else [])
    + ([80] if not DISABLE_HDIM80 else [])
...
def test_flash_attn_varlen_output(
    ...
    if not DISABLE_BACKWARD and dtype != torch.float8_e4m3fn and not has_qv and not test_sink:
...
def test_flash_attn_kvcache(
    ...
    # dv_vals = [d]
    s_aux = torch.randn(nheads, device=device, dtype=torch.bfloat16) * 4 if test_sink else None
    if dtype == torch.float8_e4m3fn and d != 192:
        dv_vals = [d]
    for dv in dv_vals:
        print("dv =", dv)
        ...
        for num_splits, precompute_metadata in itertools.product(num_splits_vals, precompute_metadata_vals):
            ...
            # print("max seqlen_q, seqlen_q ", max_seqlen_q, seqlen_q)
